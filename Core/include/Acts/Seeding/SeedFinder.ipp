...
void SeedFinder<external_spacepoint_t, platform_t>::createSeedsForGroup(
  ...
  for (const auto& spM : middleSPs) {
    ...
    // DELETE THIS!!!
    //    float R = rM;
    // if ((R > 536.79 or R < 536.75) and (R > 612.7  or R < 612.698) and (R >
    // 454.929 or R < 454.927) and (R > 531.88 or R < 531.878) ) {continue;} if
    // (R > 536.8 or R < 536.78) {continue;}
...
inline void
SeedFinder<external_spacepoint_t, platform_t>::getCompatibleDoubletsTest2(
...
template <typename out_range_t>
inline void
SeedFinder<external_spacepoint_t, platform_t>::getCompatibleDoubletsBot(
    Acts::SpacePointData& spacePointData,
    const Acts::SeedFinderOptions& options,
    const Acts::SpacePointGrid<external_spacepoint_t>& grid,
    boost::container::small_vector<Neighbour<external_spacepoint_t>, 9>&
...
inline void SeedFinder<external_spacepoint_t, platform_t>::filterCandidates(
  ...
  for (const std::size_t& b : sorted_bottoms) {
    ...
    // std::cout << "|FILTER MIDDLE + BOTTOM| " << spM.radius() << " " <<
    // (*state.compatBottomSP[b]).radius() << std::endl;
  ...
  // std::cout << "-------> rM = " << rM << std::endl;
  // std::cout << "|Seeds Map| nSeedsQ, nSeeds: " <<
  // seedFilterState.numQualitySeeds << " " << seedFilterState.numSeeds  <<
  // std::endl;
}

...
void SeedFinder<external_spacepoint_t, platform_t>::createSeedsForGroup(
  ...
  for (auto cellM : middleSPs) {
    for (auto& spM : *cellM) {
      ...
      for (auto cellT : topSPs) {
        for (auto& topSP : *cellT) {
          ...
          // the central space point SPm is in the origin of the frame and the x
          // axis points away from the interaction point in addition to a
          // translation transformation we also perform a rotation in order to
          // keep the curvature of the circle tangent to the x axis
          if (m_config.interactionPointCut) {
            ...
            if (std::abs(rM * yVal) > m_config.impactMax * xVal) {
              ...
              // can be described in terms of aCoef and bCoef, where v = aCoef *
              // u + bCoef
              ...
              // by inserting aCoef into the linear equation
              float aCoef = (vT - vIP) / (uT - uIP);
              float bCoef = vIP - aCoef * uIP;
              // the distance of the straight line from the origin (radius of
              // the circle) is related to aCoef and bCoef by d^2 = bCoef^2 / (1
...
//					state.linCircleTop.push_back(transformCoordinates(*topSP.get(), *spM, false));
					
          state.compatTopSP.push_back(topSP.get());
        }
      }
      ...
      for (auto cellB : bottomSPs) {
        for (auto& bottomSP : *cellB) {
...
//					state.linCircleBottom.push_back(transformCoordinates(*bottomSP.get(), *spM, true));
					
          state.compatBottomSP.push_back(bottomSP.get());
        }
      }
      ...
      transformCoordinates(state.compatTopSP, *spM, false, state.linCircleTop,
			...
			cotThetaSort(state.compatBottomSP, state.linCircleBottom);
			cotThetaSort(state.compatTopSP, state.linCircleTop);
      ...
      for (size_t b = 0; b < numBotSP; b++) {
        ...
        if (t0 == numTopSP) {
          break;
				...
				auto idxBSorted = idxB[b];

//        auto lb = state.linCircleBottom[idxBSorted];
...
//				std::cout << "b " << b << " idxBSorted " << idxBSorted << " cotThetaB " << cotThetaB << std::endl;
        ...
        for (size_t t = t0; t < numTopSP; t++) {
          ...
          //std::endl;
					...
					auto idxTSorted = idxT[t];

//          auto lt = state.linCircleTop[idxTSorted];
          ...
          if (m_config.useDetailedDoubleMeasurementInfo) {
            ...
            float rotationTermsUVtoXY[2] = {
                spM->x() * sinTheta / spM->radius(),
            ...
            double rMTransf[3];
            if (!xyzCoordinateCheck(m_config, spM.get(), positionMiddle,
...
//            auto spB = state.compatBottomSP[idxBSorted];
...
//            auto spT = state.compatTopSP[idxTSorted];
            ...
            rMxy = std::sqrt(rMTransf[0] * rMTransf[0] +
                             rMTransf[1] * rMTransf[1]);
          ...
          // which allows just adding the two errors if they are uncorrelated
          // (which is fair for scattering and measurement uncertainties)
          ...
          // 1/sin^4(theta) from rad to deltaCotTheta
...
//            state.topSpVec.push_back(state.compatTopSP[idxTSorted]);
...
//              *state.compatBottomSP[idxBSorted], *spM, state.topSpVec,

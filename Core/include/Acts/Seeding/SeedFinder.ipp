...
template <typename external_spacepoint_t, typename platform_t>
template <template <typename...> typename container_t, typename sp_range_t>
void SeedFinder<external_spacepoint_t, platform_t>::createSeedsForGroupSSS(
																																				const Acts::SeedFinderOptions& options, SeedingState& state,
																																				Acts::SpacePointGrid<external_spacepoint_t>& grid,
																																				std::back_insert_iterator<container_t<Seed<external_spacepoint_t>>> outIt,
																																				sp_range_t& bottomSPsIdx, sp_range_t& middleSPsIdx, sp_range_t& topSPsIdx,
	...
	for (auto& spM : middleSPs) {
		...
		// filter candidates
		filterCandidatesSSS(*spM.get(), options, seedFilterState, state);
		
		m_config.seedFilter->filterSeeds_1SpFixedSSS(
																							state.candidates_collector, outIt);
...
template <typename external_spacepoint_t, typename platform_t>
template <template <typename...> typename container_t, typename sp_range_t>
void SeedFinder<external_spacepoint_t, platform_t>::createSeedsForGroupPPP(
																																				const Acts::SeedFinderOptions& options, SeedingState& state,
																																				Acts::SpacePointGrid<external_spacepoint_t>& grid,
																																				std::back_insert_iterator<container_t<Seed<external_spacepoint_t>>> outIt,
																																				sp_range_t& bottomSPsIdx, sp_range_t& middleSPsIdx, sp_range_t& topSPsIdx,
...
//		//      }
		...
		// filter candidates
		filterCandidatesPPP(*spM.get(), options, seedFilterState, state);
		
		m_config.seedFilter->filterSeeds_1SpFixedPPP(
																							state.candidates_collector, seedFilterState.numQualitySeeds, outIt);
...
template <typename external_spacepoint_t, typename platform_t>
void SeedFinder<external_spacepoint_t, platform_t>::filterCandidatesPPP(
																																		 InternalSpacePoint<external_spacepoint_t>& spM,
																																		 const Acts::SeedFinderOptions& options, SeedFilterState& seedFilterState,
																																		 SeedingState& state) const {
	float rM = spM.radius();
	...
	for (const std::size_t& b : sorted_bottoms) {
		...
		m_config.seedFilter->filterSeeds_2SpFixedPPP(
								*state.compatBottomSP[b], spM, state.topSpVec, state.curvatures,
								state.impactParameters, seedFilterState, state.candidates_collector);
	}  // loop on bottoms
	
	//  std::cout << "nSeeds " << seedFilterState.numQualitySeeds << " "
	//            << seedFilterState.numSeeds << std::endl;
}

template <typename external_spacepoint_t, typename platform_t>
void SeedFinder<external_spacepoint_t, platform_t>::filterCandidatesSSS(
																																				InternalSpacePoint<external_spacepoint_t>& spM,
																																				const Acts::SeedFinderOptions& options, SeedFilterState& seedFilterState,
																																				SeedingState& state) const {
	float varianceRM = spM.varianceR();
	...
	for (const std::size_t& b : sorted_bottoms) {
		...
		for (size_t index_t = t0; index_t < numTopSP; index_t++) {
			...
			float rMxy = std::sqrt(rMTransf[0] * rMTransf[0] + rMTransf[1] * rMTransf[1]);
			float Ax = rMTransf[0] / rMxy;
			float Ay = rMTransf[1] / rMxy;
		...
		m_config.seedFilter->filterSeeds_2SpFixedSSS(
																								 *state.compatBottomSP[b], spM, state.topSpVec, state.curvatures,
																								 state.impactParameters, seedFilterState, state.candidates_collector);
	}  // loop on bottoms
...
template <typename sp_range_t>
std::vector<Seed<external_spacepoint_t>>
SeedFinder<external_spacepoint_t, platform_t>::createSeedsForGroupPPP(
																																	 const Acts::SeedFinderOptions& options, sp_range_t bottomSPs,
																																	 sp_range_t middleSPs, sp_range_t topSPs) const {
	SeedingState state;
	const Acts::Range1D<float> rMiddleSPRange;
	...
	createSeedsForGroupPPP(options, state, std::back_inserter(ret), bottomSPs,
											middleSPs, topSPs, rMiddleSPRange);
...
template <typename sp_range_t>
std::vector<Seed<external_spacepoint_t>>
SeedFinder<external_spacepoint_t, platform_t>::createSeedsForGroupSSS(
																																			const Acts::SeedFinderOptions& options, sp_range_t bottomSPs,
																																			sp_range_t middleSPs, sp_range_t topSPs) const {
	SeedingState state;
	const Acts::Range1D<float> rMiddleSPRange;
	...
	createSeedsForGroupSSS(options, state, std::back_inserter(ret), bottomSPs,
											middleSPs, topSPs, rMiddleSPRange);

...
*/
  ...
  //  const auto& spM = *midItr;
    ...
    getCompatibleDoubletsTest<Acts::SpacePointCandidateType::BOTTOM>(
    ...
    filterCandidatesTest(state.spacePointData, *spM.get(), options,
...
SeedFinder<external_spacepoint_t, platform_t>::getCompatibleDoublets(
  ...
  for (auto& otherSPCol : otherSPsNeighbours) {
    ...
    for (; min_itr != otherSPs.end(); ++min_itr) {
      ...
      if (candidateType == Acts::SpacePointCandidateType::BOTTOM) {
	...
	continue;
      ...
      if (not m_config.interactionPointCut) {
        ...
        if (!longitudinalCollisionRange(cotTheta, deltaZ)) {
          continue;
        }
...
template <Acts::SpacePointCandidateType candidateType, typename out_range_t>
inline void
SeedFinder<external_spacepoint_t, platform_t>::getCompatibleDoubletsTest(
    Acts::SpacePointData& spacePointData,
    const Acts::SeedFinderOptions& options,
    const Acts::SpacePointGrid<external_spacepoint_t>& grid,
    boost::container::small_vector<Neighbour<external_spacepoint_t>, 9>&
  ...
  // get number of neighbour SPs ------> does this need to be removed?
  //std::size_t nsp = 0;
  //for (const auto& otherSPCol : otherSPsNeighbours) {
  //  nsp += grid.at(otherSPCol.index).size();
  //}
  ...
  for (auto& otherSPCol : otherSPsNeighbours) {
	...
	//}
      ...
      const float test = (zM * deltaR - rM *  deltaZ);
      if (test < collisionRegionMin * deltaR ||
          test > collisionRegionMax * deltaR) {
        continue;
      }
	...
	//float cotTheta = deltaZ / deltaR;
        ...
        if (deltaZ > cotThetaMax * deltaR or deltaZ < -cotThetaMax * deltaR) {
            continue;
        }
      ...
      if (0.8* std::abs(rM * yNewFrame) > impactMax * xNewFrame) {
...
SeedFinder<external_spacepoint_t, platform_t>::longitudinalCollisionRange(
   const float& cotTheta, const float& deltaZ) const {
...
template <typename external_spacepoint_t, typename platform_t>
inline void SeedFinder<external_spacepoint_t, platform_t>::filterCandidatesTest(
    Acts::SpacePointData& spacePointData,
...
//			std::cout << " t0 " << t0 << " " << cotThetaT << " " << cotThetaB << std::endl;
...
//				std::cout << " t0 = index_t + 1 " << t0 << " = " << index_t +1 << std::endl;
        t0 = index_t + 1;
        continue;
      }
...
//  			std::cout << " t0 = index_t " << t0 << " = " << index_t << std::endl;
        t0 = index_t;
        continue;
      }
...
//			std::cout << " acepted " << std::endl;
      ...
      state.topSpVec.emplace_back(state.compatTopSP[t]);
      // inverse diameter is signed depending if the curvature is
      // positive/negative in phi
      state.curvatures.emplace_back(B / std::sqrt(S2));
      state.impactParameters.emplace_back(Im);
    }  // loop on tops

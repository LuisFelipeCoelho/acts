...
void SeedFilter<external_spacepoint_t>::filterSeeds_2SpFixed(
  ...
  for (const std::size_t topSPIndex : topSPIndexVec) {
    ...
    if (m_cfg.seedConfirmation) {
      ...
      if (deltaSeedConf > 0) {
        // if we have not yet reached our max number of quality seeds we add
        // the new seed to outCont
      ...
      // if we have not yet reached our max number of seeds we add the new
      // seed to outCont

...
ActsExamples::ProcessCode ActsExamples::SeedingAlgorithm::execute(
  ...
  for (size_t phiBin = 0; phiBin <= (size_t)nPhiBins; ++phiBin) {
    ...
    for (size_t zBin = 0; zBin < (size_t)nZBins; ++zBin) {
      ...
      //			std::cout << "topIterators.size()  " <<
      //topIterators.size() << std::endl; 			for (auto cell : topIterators) {
      //				std::cout << "cell.size()  " <<
      //cell->size() << std::endl;
      //
      //				for (auto&& spT : *cell) {
      ...
      // Skip if this particular 2D bin is empty
      //			if (spacePointsGrouping.gridBinEmpty(phiBin,
      //zBinIndex)) { 				std::cout << "TEST gridBinSize(phiBin, zBinIndex)==0" <<
      //std::endl; 				continue;
      //			}

      //			spacePointsGrouping.fastIterator(phiBin,
      //zBinIndex);
    }
  }
